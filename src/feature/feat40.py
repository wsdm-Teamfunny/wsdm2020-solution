...
def ngram_feat(df):    
    ...
    df["keyw_unigram"] = list(df.progress_apply(lambda x: preprocess_data(x["keyw"]), axis=1))
    return df
...
def count_feat(df):
    ...
    feat_names = ['quer_key', 'quer_all', 'keyw']
    grams = ['unigram']
...
#     tr = tr.head(1000)
    ...
    tr_feat = tr_feat.merge(tr_desc[['description_id', 'quer_key', 'quer_all']], on=['description_id'], how='left')

    tr_feat = multi_process_feat(tr_feat)
    del_cols = [col for col in tr_feat.columns if 'keyw' not in col and col not in ID_NAMES]
...
if __name__ == "__main__":
    ...
    paper['keyw'] = paper['keywords'].fillna('').replace(';', ' ')
    
    tr_desc_path = '../../input/tr_input_final.ftr'
    te_desc_path = '../../input/te_input_final.ftr'

    tr_recall_path = '../../feat/tr_s0_38.ftr'
    te_recall_path = '../../feat/te_s0_38.ftr'
