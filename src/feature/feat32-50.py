...
paper_ids = pickle.load(open('../../feat/paper_ids.pkl', 'rb'))
paper_corpus = pickle.load(open('../../feat/paper_corpus.pkl', 'rb'))
paper_abst = pickle.load(open('../../feat/paper_abst.pkl', 'rb'))
paper_titl = pickle.load(open('../../feat/paper_titl.pkl', 'rb'))

bm25_corp = bm25.BM25(paper_corpus)
bm25_abst = bm25.BM25(paper_abst)
bm25_titl = bm25.BM25(paper_titl)
...
#     print (i, 'tfidf completed, cost {}s'.format(np.round(time.time() - ts), 2))
#     for dis_name in ['cos', 'eucl', 'manh']:
#         metric_name = 'tfidf'
#         df['max_{}_{}_dis'.format(metric_name, dis_name)] = df.progress_apply(lambda row: \
#                 max(row['quer_key_titl_tfidf_{}_dis'.format(dis_name)], row['quer_key_abst_tfidf_{}_dis'.format(dis_name)]), axis=1)
#         df['min_{}_{}_dis'.format(metric_name, dis_name)] = df.progress_apply(lambda row: \
#                 min(row['quer_key_titl_tfidf_{}_dis'.format(dis_name)], row['quer_key_abst_tfidf_{}_dis'.format(dis_name)]), axis=1)
#         df['sum_{}_{}_dis'.format(metric_name, dis_name)] = df.progress_apply(lambda row: \
#                 sum_score(row['quer_key_titl_tfidf_{}_dis'.format(dis_name)], \
#                 row['quer_key_abst_tfidf_{}_dis'.format(dis_name)]), axis=1)          
    
    # bm25 score
    for col_name in ['quer_key', 'quer_all']:
        df['{}_corp_bm25_score'.format(col_name)] = df.progress_apply(lambda row: \
                get_bm25_corp(row[col_name], row['paper_id']), axis=1)
        df['{}_abst_bm25_score'.format(col_name)] = df.progress_apply(lambda row: \
                get_bm25_abst(row[col_name], row['paper_id']), axis=1)
        df['{}_titl_bm25_score'.format(col_name)] = df.progress_apply(lambda row: \
                get_bm25_titl(row[col_name], row['paper_id']), axis=1)
        
    print (i, 'bm25 completed, cost {}s'.format(np.round(time.time() - ts), 2))
...
if __name__ == "__main__":
    ...
    tr_recall_path = '../../feat/tr_s0_31-50.ftr'
    te_recall_path = '../../feat/te_s0_31-50.ftr'
