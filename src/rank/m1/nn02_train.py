...
for fea in tqdm(feature_columns):
    if train_test[fea].max()<=1.0:continue
        ...
        train_test[fea] = MinMaxScaler().fit_transform(train_test[fea].values.reshape(-1,1))
...
#                     validation_data=(valid_x,valid_y), 
#                     callbacks=[es, rlr, sv_mod], epochs=1000, batch_size=2048, verbose=1)
        cv_predict=model.predict(valid_x)
...
# result = []
# for i in range(1,6):
#     re_csv = f"../../../output/m1/nn02_result_{i}.csv"
...
# final_test.to_csv("../../../output/m1/nn02/final_test.csv",index=False)
...
# from tqdm import tqdm
# fo = open("../../../output/m1/nn02/sub_nn02.csv", 'w+')
